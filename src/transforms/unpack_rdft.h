...
class InvalidPackedWindowSizeException : public ExceptionBase {
 public:
  explicit InvalidPackedWindowSizeException(size_t size)
  : ExceptionBase("Size " + std::to_string(size) +
                  " is not recognized. It must be either a power of 2 plus 1 "
                  "(real case) or a power of 2 plus 2 (complex case).") {}
};
